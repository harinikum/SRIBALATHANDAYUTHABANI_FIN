...
const CustomerDetailsRight = ({values, setValues=()=>{}, buttons=[], setButtons=()=>{},custDetailArrState, setCustDetailArrState, afterMembers, setAfterMembers, datas, setDatas, hasMore, setHasMore}) => {
  ...
  const openFun = async()=>{
    ...
    const res = await tableFectchApi({data : datas, setData : setDatas, filters : filters, setHasMore : setHasMore, URL : endPointURLs.getMembers, method : "POST"});
    setLoaded(true);
  }
  ...
  const scrollFunction = async(filters={})=>{
    ...
    await tableFectchApi({data : datas, setData:setDatas , filters : {...cleanFilters, agent_name : values.agent || agentName || "All"}, setHasMore : setHasMore,URL : endPointURLs.getMembers, method : "POST"})
    setLoaded(true);
